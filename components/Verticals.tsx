...
const verticals: Vertical[] = [
  { id: 1, title: 'Artificial Intelligence', icon: Brain, color: 'from-blue-500 to-purple-500' },
  { id: 2, title: 'Deep Tech Startups', icon: Rocket, color: 'from-purple-500 to-pink-500' },
  { id: 3, title: 'Global Innovation', icon: Globe, color: 'from-green-500 to-teal-500' },
  { id: 4, title: 'Quantum Computing', icon: Network, color: 'from-yellow-500 to-orange-500' },
  { id: 5, title: 'Sustainable Tech', icon: Recycle, color: 'from-emerald-500 to-green-500' },
  { id: 6, title: 'Robotics & Automation', icon: Bot, color: 'from-red-500 to-pink-500' },
];
    ...
    <div className={`min-h-screen ${preview ? 'pt-10 pb-10' : 'pt-24 pb-20'} px-6 md:px-20 bg-black`}>
      ...
      >
        <div className="flex items-center gap-4">
            ...
            <span className="text-lg text-gray-300 uppercase tracking-widest">Core Tracks</span>
        </div>
         {preview && onViewAll && (
            <button 
                onClick={onViewAll}
      ...
      <div className={`grid grid-cols-2 md:grid-cols-3 ${preview ? 'lg:grid-cols-6' : 'lg:grid-cols-6'} gap-6`}>
        {(preview ? verticals.slice(0, 6) : verticals).map((v, i) => (
          <motion.div
            ...
            className="group relative h-64 md:h-80 rounded-[30px] border border-white/10 bg-white/5 overflow-hidden cursor-pointer hover:border-white/30 transition-all duration-300"
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
              <div className="p-4 rounded-full bg-white/5 mb-6 group-hover:scale-110 transition-transform duration-300">
                <v.icon size={32} className="text-white" strokeWidth={1.5} />
              </div>
              <h3 className="text-sm font-medium text-gray-300 group-hover:text-white transition-colors">{v.title}</h3>
            </div>
            
            {/* Inner Glow */}
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-20 h-20 bg-white/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </motion.div>
        ))}
      </div>
